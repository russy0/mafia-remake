...
try {
    Invoke-MafiaNative $Rustup run $Toolchain cargo build --release --bin mafia
    $BuiltExe = Join-Path $RustRoot "target\release\mafia.exe"
    $RepoExeDir = Join-Path $BuildRepoRoot "target\release"
    New-Item -ItemType Directory -Force -Path $RepoExeDir | Out-Null
    Copy-Item -LiteralPath $BuiltExe -Destination (Join-Path $RepoExeDir "mafia.exe") -Force
} finally {
    Pop-Location
}
} finally {
