...
AArch64 Linux용 바이너리는 전용 스크립트로 빌드합니다. 기본은 musl 정적 바이너리입니다.

```bash
./scripts/build-linux-aarch64.sh
```

결과물은 `./target/aarch64-unknown-linux-musl/release/mafia`입니다. glibc 동적 링크가 필요하면 다음처럼 실행합니다.

```bash
./scripts/build-linux-aarch64.sh glibc
```

glibc 결과물은 `./target/aarch64-unknown-linux-gnu/release/mafia`입니다. x86_64 머신에서 aarch64 glibc로 크로스 빌드하려면 `sudo apt install -y gcc-aarch64-linux-gnu`가 필요합니다.
